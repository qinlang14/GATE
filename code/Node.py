...
from module import ModalityAttentionLayer
from utils import smooth_labels, bi_tempered_logistic_loss
import numpy as np
from typing import List, Union

class NodeSelector(nn.Module):
    def __init__(self, hidden_dim, environment, gvt, mask, propagation_rate):
        super().__init__()
        self.device = "cuda:0"
        ...
        self.modality_attention_layer = ModalityAttentionLayer(int(self.hidden_dim / 2))
    ...
    # def multi_root_select(self, agent_state: torch.Tensor, roots: List[List[str]], nodes: List[List[str]],
    #                       embeddings: torch.Tensor) -> np.ndarray:
    #     for i in range(len(roots)):
    #         if len(roots[i]) > 1:
    #             node_embedding = torch.stack(
    #                 [embeddings[i][list(nodes[i]).index(roots[i][j])] for j in range(len(roots[i]))])
    #             score = torch.sum(agent_state[i] * node_embedding, dim=1)
    #             idx = int(torch.max(score, dim=-1)[1])
    #             roots[i] = [roots[i][idx]]
    #
    #     return np.array(sum(roots, []))
    ...
    def node_score(self, all_node_embedding: torch.Tensor, state: torch.Tensor, node_idx: torch.Tensor, nodes: np.ndarray,
        ...
        # if True in torch.isnan(score) or True in torch.isinf(score):
        #     print()
    ...
    def walk_step(self, nodes, adj, label, all_node_embedding, input_info: torch.Tensor, agent_state: torch.Tensor,
        ...
        # if type(current_node[0]) == list:
        #     # List[List[str]] -> np.ndarray[str], 部分样本有多个根节点，根据点积得分取其一
        #     current_node = self.multi_root_select(agent_state=agent_state, roots=current_node, nodes=nodes, embeddings = all_node_embedding)
        ...
        # nll = nll_loss(label, current_score)
